...
var createQuiz = function(quizData) {
  ...
  function showResults() {
    ...
    var imgURL = "http://media.jrn.com/images/js-for-social-200-final.jpg";
    var facebookURL = 'https://www.facebook.com/dialog/feed?display=popup&app_id=227386193957280&link=' + window.location.origin + window.location.pathname + '?qs=fb&picture=' + imgURL + '&name=&description=I+scored+' + correct + '/' + questions.length + '+on+the+JSOnline.com+News+Quiz.&redirect_uri=' + window.location.origin + window.location.pathname + '?qs=fb';
    var twitterURL = 'http://twitter.com/share?url=' + window.location.origin + window.location.pathname + '?qs=tw&text=I+scored+' + correct + '/' + questions.length + '+on+the+JSOnline.com+News+Quiz.';
    var facebook = '<a href="' + facebookURL + '" onclick="if(typeof sendGAevents == \'function\') {sendGAevents(\'JSO Quiz\', \'Results - Facebook Share\', ' + correct + ');};"><button class="q-quiz-button q-shareBtn q-facebook">facebook</button></a>';
    var twitter = '<a href="' + twitterURL + '" onclick="if(typeof sendGAevents == \'function\') {sendGAevents(\'JSO Quiz\', \'Results - Twitter Share\', ' + correct + ');};"><button class="q-quiz-button q-shareBtn q-twitter">Twitter</button></a>';
  ...
  try {
    ...
    // render questions
    _.each(quizData, function(question, index){
      var data = {index: index, question: question};
      quiz.innerHTML = quiz.innerHTML + JST.generateQuestion(data);
    });
    ...
    next.addEventListener('click', function(){
      ...
      if(loadAnswer(currentQuestion)) {
        ...
        _.each(opts.getElementsByClassName('q-option'), function(opt, index) {
          var id = ((currentQuestion * index) + index);
          options[id].removeEventListener('click', listener);
        ...
        try {
          ...
          console.log('JSO Quiz error: (next)' + err);
        }
      }
    ...
    prev.addEventListener('click', function(){
      ...
      if(loadAnswer(currentQuestion)) {
        ...
        try {
          ...
          console.log('JSO Quiz error: (prev)' + err);
        }
      }
    ...
    _.each(options,function(option) {
      ...
      option.addEventListener('click', listener, false);
    });
    ...
    if(loadAnswer(currentQuestion)) {
      ...
      try {
        ...
        console.log('JSO Quiz error: (answerMarking)' + err);
      }
    }
  } catch (err) {
    console.log('JSO Quiz error: (main)' + err);
  }
};
//
